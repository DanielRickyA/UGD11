...
const Game1 = () => {
  ...
  const [nilai, setNilai] = useState(Math.floor(Math.random() * 10) + 21);
  ...
  const cekTebakan = (event) => {
    ...
    if(jumlahTebakan > -1){
      if (jumlahTebakan === 4 && value !== nilai) {
        toast.error("Game Sudah berakhir");
    ...
    if (value === nilai) {
      ...
      setNilai(Math.floor(Math.random() * 10) + 21);
      setJumlahTebakan(0);
    } else if (tebakan != "") {
  ...
  return (
    <div className="p-4">
      ...
      <ol className="text-start m-2 mt-4">
        ...
        <li>Number range is between 21 - 30</li>
          ...
          {jumlahTebakan === -1 || (
            <>
              ...
              <div className="d-flex m">
                ...
                />
                <div>
                  <button type="button" onClick={changeShow} className="btn btn-primary ms-2">
